#-----------------------------------------------------------------
# semanticscience::org::resource::SIO_000105
# Generated: 12-Aug-2010 09:35:07 PDT
# Contact: Edward Kawas <edward.kawas+owl2perl@gmail.com>
#-----------------------------------------------------------------
package semanticscience::org::resource::SIO_000105;

use OWL::Data::OWL::Class;
use semanticscience::org::resource::SIO_000361;
...
@ISA = qw(
 OWL::Data::OWL::Class
 semanticscience::org::resource::SIO_000361 
);
use strict;
...
use semanticscience::org::resource::SIO_000210;
...

{
    my %_allowed = (
        ...
        SIO_000210 => {
        	...
        	post => sub {
                ...
                my $type = @{$self->SIO_000210}[-1] if defined $self->SIO_000210;
                return unless defined $type and $type->type;
                my $range = new semanticscience::org::resource::SIO_000210->range();
                return unless $range;
    ...
    sub _accessible {
        ...
        exists $_allowed{$attr} 
          ...
          or $self->semanticscience::org::resource::SIO_000361::_accessible($attr)
    }
    ...
    sub _attr_prop {
        ...
        return $self->semanticscience::org::resource::SIO_000361::_attr_prop( $attr_name, $prop_name ) 
            if $self->semanticscience::org::resource::SIO_000361::_accessible($attr_name);
...
sub init {
    ...
    $self->semanticscience::org::resource::SIO_000361::init();
    $self->type('http://semanticscience.org/resource/SIO_000105');
...
sub _get_statements {
    ...
    eval {
    	# add parent statements
        $self->semanticscience::org::resource::SIO_000361::_get_statements();
    };

    # for each attribute, if is array do:
    if (defined $self->SIO_000210){
	    foreach (@{$self->SIO_000210}) {
	        ...
	        $self->model->addStmt(
	          new RDF::Core::Statement(
	              $subject,
	              $subject->new('http://semanticscience.org/resource/SIO_000210'), 
...
semanticscience::org::resource::SIO_000105 - an automatically generated owl class!

=cut

=head1 SYNOPSIS

  use semanticscience::org::resource::SIO_000105;
  my $class = semanticscience::org::resource::SIO_000105->new();
  ...
  $class->add_SIO_000210(new OWL::Data::OWL::Class('#someURI'));

  # get the SIO_000210 object properties
  my $SIO_000210_objects = $class->SIO_000210;
  # Make sure to use the appropriate OWL class! 
  # OWL::Data::OWL::Class used as an example
...
L<semanticscience::org::resource::SIO_000361>
