#-----------------------------------------------------------------
# Service name: getKeggPathwaysByEnzyme
# Authority:    dev.biordf.net
# Created:      13-May-2010 09:44:43 PDT
# Contact:      edward.kawas@gmail.com
# Description:  
#               takes an enzyme record and retrieves the pathways that are associated with this enzyme
#-----------------------------------------------------------------

package Service::getKeggPathwaysByEnzyme;
...
BEGIN {
    ...
    new SADI::Generators::GenServices->async_load(
	 service_names => ['getKeggPathwaysByEnzyme']);
}

# (this to stay here with any of the options above)
use vars qw( @ISA );
@ISA = qw( net::biordf::dev::getKeggPathwaysByEnzymeBase );
use strict;
...
sub process_it {
    ...
    foreach my $input (@inputs) {
    	...
    	unless ($accession) {
	        ...
	        if ($accession =~ m/ENZYME:(.*)/gi) {
	            $accession = $1;    
	        } else {
	            next;
	        }
    	}
        my $class = Implementations::getKeggPathwaysByEnzyme($input, $accession, $LOG);
...
Service::getKeggPathwaysByEnzyme - a SADI service
