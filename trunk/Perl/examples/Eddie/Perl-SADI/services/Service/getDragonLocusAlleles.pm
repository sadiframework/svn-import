...
BEGIN {
    ...
    new SADI::Generators::GenServices->async_load(
	 service_names => ['getDragonLocusAlleles']);
}

# (this to stay here with any of the options above)
use vars qw( @ISA );
@ISA = qw( net::antirrhinum::getDragonLocusAllelesBase );
use strict;
...
sub process_it {
    ...
    foreach my $input (@inputs) {
        ...
        unless ($id) {
            ...
            # example: http://lsrn.org/DragonDB_Locus:cho
            $id = $1 if $id =~ /.*#(\w+)$/;
            $id = $1 if $id =~ /.*\/(\w+)$/;
            $id = $1 if $id =~ /.*:(\w+)$/;
        }
	    my $class = Implementations::getDragonLocusAlleles($input, $id, $LOG); 
...
Service::getDragonLocusAlleles - a SADI service
