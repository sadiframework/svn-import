...
BEGIN {
    ...
    new SADI::Generators::GenServices->async_load(
	 service_names => ['getEnzymeGeneComponentPartsMouse']);
}

# (this to stay here with any of the options above)
use vars qw( @ISA );
@ISA = qw( net::biordf::dev::getEnzymeGeneComponentPartsMouseBase );
use strict;
...
sub process_it {
   my ($self, $values, $core) = @_;
    ...
    foreach my $input (@inputs) {
        ...
        my $class = Implementations::getEnzymeGeneComponentPartsMouse($input, $accession, $LOG);
...
Service::getEnzymeGeneComponentPartsMouse - a SADI service
