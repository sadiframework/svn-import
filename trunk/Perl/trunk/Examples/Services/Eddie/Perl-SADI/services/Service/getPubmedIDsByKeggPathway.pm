#-----------------------------------------------------------------
# Service name: getPubmedIDsByKeggPathway
# Authority:    dev.biordf.net
# Created:      14-Jan-2010 11:14:04 PST
# Contact:      edward.kawas+getPubmedIDsByKeggPathway@gmail.com
# Description:  
#               Returns all PubMed IDs of the references associated with the specified (KEGG) pathway.
#-----------------------------------------------------------------

package Service::getPubmedIDsByKeggPathway;
...
BEGIN {
    ...
    new SADI::Generators::GenServices->async_load(
	 service_names => ['getPubmedIDsByKeggPathway']);
}

# (this to stay here with any of the options above)
use vars qw( @ISA );
@ISA = qw( net::biordf::dev::getPubmedIDsByKeggPathwayBase );
use strict;
...
sub process_it {
    ...
    foreach my $input (@inputs) {
        ...
        unless ($accession) {
	        ...
	        $accession = "$1" if $accession =~ m/KEGG_PATHWAY\:(.*)/gi;
        }    	
    	my $class = Implementations::getPubmedIDsByKeggPathway($input, $accession, $LOG);
...
Service::getPubmedIDsByKeggPathway - a SADI service
...
B<Email>: edward.kawas+getPubmedIDsByKeggPathway@gmail.com
