#-----------------------------------------------------------------
# Service name: getEcGeneComponentParts
# Authority:    dev.biordf.net
# Created:      15-Jun-2010 12:10:06 PDT
# Contact:      edward.kawas+sadiservice@gmail.com
# Description:  
#               Using TOGO WS, fetch the enzymes component KEGG genes
#-----------------------------------------------------------------

package Service::getEcGeneComponentParts;
...
BEGIN {
    ...
    new SADI::Generators::GenServices->async_load(
	 service_names => ['getEcGeneComponentParts']);
}

# (this to stay here with any of the options above)
use vars qw( @ISA );
@ISA = qw( net::biordf::dev::getEcGeneComponentPartsBase );
use strict;
...
sub process_it {
    ...
    foreach my $input (@inputs) {
        ...
        unless ($accession) {
            ...
            $accession = "$1" if $accession =~ m/EC\:(.*)/gi;
            next if $accession eq $input->getURI;
        }     
        my $class = Implementations::getEcGeneComponentParts($input, $accession, $LOG);
...
Service::getEcGeneComponentParts - a SADI service
...
B<Email>: edward.kawas+sadiservice@gmail.com
