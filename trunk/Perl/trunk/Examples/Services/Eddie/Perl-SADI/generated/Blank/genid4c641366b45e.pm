#-----------------------------------------------------------------
# Blank::genid4c641366b45e
# Generated: 12-Aug-2010 09:34:45 PDT
# Contact: Edward Kawas <edward.kawas+owl2perl@gmail.com>
#-----------------------------------------------------------------
package Blank::genid4c641366b45e;

use OWL::Data::OWL::Class;
use semanticscience::org::resource::SIO_010347;
...
@ISA = qw(
 OWL::Data::OWL::Class
 semanticscience::org::resource::SIO_010347 
);
use strict;
...
use semanticscience::org::resource::SIO_000356;
...

{
    my %_allowed = (
        ...
        SIO_000356 => {
        	...
        	post => sub {
                ...
                my $type = @{$self->SIO_000356}[-1] if defined $self->SIO_000356;
                return unless defined $type and $type->type;
                my $range = new semanticscience::org::resource::SIO_000356->range();
                return unless $range;
    ...
    sub _accessible {
        ...
        exists $_allowed{$attr} 
          ...
          or $self->semanticscience::org::resource::SIO_010347::_accessible($attr)
    }
    ...
    sub _attr_prop {
        ...
        return $self->semanticscience::org::resource::SIO_010347::_attr_prop( $attr_name, $prop_name ) 
            if $self->semanticscience::org::resource::SIO_010347::_accessible($attr_name);
...
sub init {
    ...
    $self->semanticscience::org::resource::SIO_010347::init();
    $self->type('_:genid4c641366b45e');
...
sub _get_statements {
    ...
    eval {
    	# add parent statements
        $self->semanticscience::org::resource::SIO_010347::_get_statements();
    };

    # for each attribute, if is array do:
    if (defined $self->SIO_000356){
	    foreach (@{$self->SIO_000356}) {
	        ...
	        $self->model->addStmt(
	          new RDF::Core::Statement(
	              $subject,
	              $subject->new('http://semanticscience.org/resource/SIO_000356'), 
...
Blank::genid4c641366b45e - an automatically generated owl class!

=cut

=head1 SYNOPSIS

  use Blank::genid4c641366b45e;
  my $class = Blank::genid4c641366b45e->new();

  # get the uri for this class
  my $uri = $class->uri;
  # add object properties 
  use semanticscience::org::resource::SIO_010012;
  # add a SIO_000356 property (semanticscience::org::resource::SIO_010012)
  $class->add_SIO_000356(new semanticscience::org::resource::SIO_010012());
  # get the SIO_000356 object properties
  my $SIO_000356_objects = $class->SIO_000356;
...
L<semanticscience::org::resource::SIO_010347>
