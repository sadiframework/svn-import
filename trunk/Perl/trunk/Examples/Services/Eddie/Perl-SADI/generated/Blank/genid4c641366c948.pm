#-----------------------------------------------------------------
# Blank::genid4c641366c948
# Generated: 12-Aug-2010 09:33:28 PDT
# Contact: Edward Kawas <edward.kawas+owl2perl@gmail.com>
#-----------------------------------------------------------------
package Blank::genid4c641366c948;
...
use semanticscience::org::resource::SIO_000310;
use semanticscience::org::resource::SIO_000335;
    ...
    my %_allowed = (
        ...
        SIO_000310 => {
        	...
        	post => sub {
                ...
                my $type = @{$self->SIO_000310}[-1] if defined $self->SIO_000310;
                return unless defined $type and $type->type;
                my $range = new semanticscience::org::resource::SIO_000310->range();
                return unless $range;
        ...
        SIO_000335 => {
        	...
        	post => sub {
        		my ($self) = shift;
        		return unless $self->strict;
                my $type = @{$self->SIO_000335}[-1] if defined $self->SIO_000335;
                return unless defined $type and $type->type;
                my $range = new semanticscience::org::resource::SIO_000335->range();
                return unless $range;
                $range = $self->uri2package($range);
                eval {
                	$range = $range->new();
...
sub _get_statements {
    ...
    if (defined $self->SIO_000310){
	    foreach (@{$self->SIO_000310}) {
	        ...
	        $self->model->addStmt(
	          new RDF::Core::Statement(
	              $subject,
	              $subject->new('http://semanticscience.org/resource/SIO_000310'), 
    ...
    # for each attribute, if is array do:
    if (defined $self->SIO_000335){
	    foreach (@{$self->SIO_000335}) {
	        # add all statements from $_
	        my $enumerator = $_->_get_statements if defined $_->_get_statements;
	        next unless defined $enumerator;
            my $statement = $enumerator->getFirst;
	        ...
	        $self->model->addStmt(
	          new RDF::Core::Statement(
	              $subject,
	              $subject->new('http://semanticscience.org/resource/SIO_000335'), 
	              (defined $_->subject ? $_->subject : RDF::Core::Resource->new($_->value))
	          )
	        );
	        $add_type_statement = 1;
...
Blank::genid4c641366c948 - an automatically generated owl class!

=cut

=head1 SYNOPSIS

  use Blank::genid4c641366c948;
  my $class = Blank::genid4c641366c948->new();

  # get the uri for this class
  my $uri = $class->uri;
  # add object properties 
  use semanticscience::org::resource::SIO_010506;
  # add a SIO_000310 property (semanticscience::org::resource::SIO_010506)
  $class->add_SIO_000310(new semanticscience::org::resource::SIO_010506());
  # get the SIO_000310 object properties
  my $SIO_000310_objects = $class->SIO_000310;
  use semanticscience::org::resource::SIO_011125;
  # add a SIO_000335 property (semanticscience::org::resource::SIO_011125)
  $class->add_SIO_000335(new semanticscience::org::resource::SIO_011125());
  # get the SIO_000335 object properties
  my $SIO_000335_objects = $class->SIO_000335;
