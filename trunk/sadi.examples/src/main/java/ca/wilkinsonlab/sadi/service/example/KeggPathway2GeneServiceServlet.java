...
public class KeggPathway2GeneServiceServlet extends SimpleAsynchronousServiceServlet
{
	...
	protected void processInput(Resource input, Resource output)
	{
		...
		try {
			...
			keggGeneIds = new KEGGLocator().getKEGGPort().get_genes_by_pathway(String.format("path:%s", keggPathwayId));
