#-----------------------------------------------------------------
# semanticscience::org::resource::SIO_011121
# Generated: 12-Aug-2010 09:35:11 PDT
# Contact: Edward Kawas <edward.kawas+owl2perl@gmail.com>
#-----------------------------------------------------------------
package semanticscience::org::resource::SIO_011121;

use OWL::Data::OWL::Class;
use semanticscience::org::resource::SIO_011123;
...
@ISA = qw(
 OWL::Data::OWL::Class
 semanticscience::org::resource::SIO_011123 
);
use strict;
...
use semanticscience::org::resource::SIO_000146;
use semanticscience::org::resource::SIO_000338;
...

{
    my %_allowed = (
        ...
        SIO_000146 => {
        	...
        	post => sub {
                ...
                my $type = @{$self->SIO_000146}[-1] if defined $self->SIO_000146;
                return unless defined $type and $type->type;
                my $range = new semanticscience::org::resource::SIO_000146->range();
                return unless $range;
        ...
        SIO_000338 => {
        	...
        	post => sub {
        		my ($self) = shift;
        		return unless $self->strict;
                my $type = @{$self->SIO_000338}[-1] if defined $self->SIO_000338;
                return unless defined $type and $type->type;
                my $range = new semanticscience::org::resource::SIO_000338->range();
                return unless $range;
                $range = $self->uri2package($range);
                eval {
                	$range = $range->new();
    ...
    sub _accessible {
        ...
        exists $_allowed{$attr} 
          ...
          or $self->semanticscience::org::resource::SIO_011123::_accessible($attr)
    }
    ...
    sub _attr_prop {
        ...
        return $self->semanticscience::org::resource::SIO_011123::_attr_prop( $attr_name, $prop_name ) 
            if $self->semanticscience::org::resource::SIO_011123::_accessible($attr_name);
...
sub init {
    ...
    $self->semanticscience::org::resource::SIO_011123::init();
    $self->type('http://semanticscience.org/resource/SIO_011121');
...
sub _get_statements {
    ...
    eval {
    	# add parent statements
        $self->semanticscience::org::resource::SIO_011123::_get_statements();
    };

    # for each attribute, if is array do:
    if (defined $self->SIO_000146){
	    foreach (@{$self->SIO_000146}) {
	        ...
	        $self->model->addStmt(
	          new RDF::Core::Statement(
	              $subject,
	              $subject->new('http://semanticscience.org/resource/SIO_000146'), 
    ...
    # for each attribute, if is array do:
    if (defined $self->SIO_000338){
	    foreach (@{$self->SIO_000338}) {
	        # add all statements from $_
	        my $enumerator = $_->_get_statements if defined $_->_get_statements;
	        next unless defined $enumerator;
            my $statement = $enumerator->getFirst;
	        ...
	        $self->model->addStmt(
	          new RDF::Core::Statement(
	              $subject,
	              $subject->new('http://semanticscience.org/resource/SIO_000338'), 
	              (defined $_->subject ? $_->subject : RDF::Core::Resource->new($_->value))
	          )
	        );
	        $add_type_statement = 1;
...
semanticscience::org::resource::SIO_011121 - an automatically generated owl class!

=cut

=head1 SYNOPSIS

  use semanticscience::org::resource::SIO_011121;
  my $class = semanticscience::org::resource::SIO_011121->new();
  ...
  $class->add_SIO_000146(new OWL::Data::OWL::Class('#someURI'));

  # get the SIO_000146 object properties
  my $SIO_000146_objects = $class->SIO_000146;
  # Make sure to use the appropriate OWL class! 
  # OWL::Data::OWL::Class used as an example
  $class->add_SIO_000338(new OWL::Data::OWL::Class('#someURI'));

  # get the SIO_000338 object properties
  my $SIO_000338_objects = $class->SIO_000338;
...
L<semanticscience::org::resource::SIO_011123>
