#-----------------------------------------------------------------
# semanticscience::org::resource::SIO_010016
# Generated: 12-Aug-2010 09:35:32 PDT
# Contact: Edward Kawas <edward.kawas+owl2perl@gmail.com>
#-----------------------------------------------------------------
package semanticscience::org::resource::SIO_010016;

use OWL::Data::OWL::Class;
use semanticscience::org::resource::SIO_000030;
...
@ISA = qw(
 OWL::Data::OWL::Class
 semanticscience::org::resource::SIO_000030 
);
use strict;
...
use semanticscience::org::resource::SIO_000011;
...

{
    my %_allowed = (
        ...
        SIO_000011 => {
        	...
        	post => sub {
                ...
                my $type = @{$self->SIO_000011}[-1] if defined $self->SIO_000011;
                return unless defined $type and $type->type;
                my $range = new semanticscience::org::resource::SIO_000011->range();
                return unless $range;
    ...
    sub _accessible {
        ...
        exists $_allowed{$attr} 
          ...
          or $self->semanticscience::org::resource::SIO_000030::_accessible($attr)
    }
    ...
    sub _attr_prop {
        ...
        return $self->semanticscience::org::resource::SIO_000030::_attr_prop( $attr_name, $prop_name ) 
            if $self->semanticscience::org::resource::SIO_000030::_accessible($attr_name);
...
sub init {
    ...
    $self->semanticscience::org::resource::SIO_000030::init();
    $self->type('http://semanticscience.org/resource/SIO_010016');
...
sub _get_statements {
    ...
    eval {
    	# add parent statements
        $self->semanticscience::org::resource::SIO_000030::_get_statements();
    };

    # for each attribute, if is array do:
    if (defined $self->SIO_000011){
	    foreach (@{$self->SIO_000011}) {
	        ...
	        $self->model->addStmt(
	          new RDF::Core::Statement(
	              $subject,
	              $subject->new('http://semanticscience.org/resource/SIO_000011'), 
...
semanticscience::org::resource::SIO_010016 - an automatically generated owl class!

=cut

=head1 SYNOPSIS

  use semanticscience::org::resource::SIO_010016;
  my $class = semanticscience::org::resource::SIO_010016->new();
  ...
  $class->add_SIO_000011(new OWL::Data::OWL::Class('#someURI'));

  # get the SIO_000011 object properties
  my $SIO_000011_objects = $class->SIO_000011;
  # Make sure to use the appropriate OWL class! 
  # OWL::Data::OWL::Class used as an example
...
L<semanticscience::org::resource::SIO_000030>
