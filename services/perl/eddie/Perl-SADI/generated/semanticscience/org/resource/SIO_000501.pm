#-----------------------------------------------------------------
# semanticscience::org::resource::SIO_000501
# Generated: 12-Aug-2010 09:34:56 PDT
# Contact: Edward Kawas <edward.kawas+owl2perl@gmail.com>
#-----------------------------------------------------------------
package semanticscience::org::resource::SIO_000501;
...
use semanticscience::org::resource::SIO_000068;
    ...
    my %_allowed = (
        ...
        SIO_000068 => {
        	...
        	post => sub {
        		my ($self) = shift;
        		return unless $self->strict;
                my $type = @{$self->SIO_000068}[-1] if defined $self->SIO_000068;
                return unless defined $type and $type->type;
                my $range = new semanticscience::org::resource::SIO_000068->range();
                return unless $range;
                $range = $self->uri2package($range);
                eval {
                	$range = $range->new();
...
sub init {
    ...
    $self->type('http://semanticscience.org/resource/SIO_000501');
...
sub _get_statements {
    ...
    # for each attribute, if is array do:
    if (defined $self->SIO_000068){
	    foreach (@{$self->SIO_000068}) {
	        # add all statements from $_
	        my $enumerator = $_->_get_statements if defined $_->_get_statements;
	        next unless defined $enumerator;
            my $statement = $enumerator->getFirst;
	        ...
	        $self->model->addStmt(
	          new RDF::Core::Statement(
	              $subject,
	              $subject->new('http://semanticscience.org/resource/SIO_000068'), 
	              (defined $_->subject ? $_->subject : RDF::Core::Resource->new($_->value))
	          )
	        );
	        $add_type_statement = 1;
...
semanticscience::org::resource::SIO_000501 - an automatically generated owl class!

=cut

=head1 SYNOPSIS

  use semanticscience::org::resource::SIO_000501;
  my $class = semanticscience::org::resource::SIO_000501->new();
  ...
  # Make sure to use the appropriate OWL class! 
  # OWL::Data::OWL::Class used as an example
  $class->add_SIO_000068(new OWL::Data::OWL::Class('#someURI'));

  # get the SIO_000068 object properties
  my $SIO_000068_objects = $class->SIO_000068;
