...
BEGIN {
    ...
    new SADI::Generators::GenServices->async_load(
	 service_names => ['getEnzymesEncodedByKeggGenes']);
}

# (this to stay here with any of the options above)
use vars qw( @ISA );
@ISA = qw( net::biordf::dev::getEnzymesEncodedByKeggGenesBase );
use strict;
...
sub process_it {
    ...
    foreach my $input (@inputs) {
		...
		my $class = Implementations::getEnzymesEncodedByKeggGenes($input, $accession, $LOG);
		$core->addOutputData(
                node => $class,
        );
    }
...
Service::getEnzymesEncodedByKeggGenes - a SADI service
