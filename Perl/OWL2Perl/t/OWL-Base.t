...
is(ref $data, "OWL::Data::Object", 'is object reference a OWL::Data::Object?');
...
# test primed boolean
$data = OWL::Data::Boolean->new (namespace=>"NCBI_gi", id=>"545454", value=>1);
is(ref $data, "OWL::Data::Boolean", 'check object reference (OWL::Data::Boolean)');
...
$data = OWL::Data::DateTime->new (namespace=>"NCBI_gi", id=>"545454", value=>'2009-07-15 19:44:55Z');
is(ref $data, "OWL::Data::DateTime", 'check object reference (OWL::Data::DateTime)');
can_ok($data, ('toString','id', 'namespace', 'value'));
ok($data->id eq '545454', 'Check id we primed this object with');
ok($data->namespace eq 'NCBI_gi', 'Check namespace we primed this object with');
ok($data->primitive, 'Check if we are a primitive');
...
is(ref $data, "OWL::Data::DateTime", 'check object reference for incorrectly primed object (OWL::Data::DateTime)');
can_ok($data, ('toString','id', 'namespace', 'value'));
ok($data->id eq '', 'Check id (should be empty)');
ok($data->namespace eq '', 'Check namespace (should be empty)');
ok($data->primitive, 'Check if we are a primitive');
