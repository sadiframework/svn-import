#-----------------------------------------------------------------
# semanticscience::org::resource::SIO_000500
# Generated: 12-Aug-2010 09:34:17 PDT
# Contact: Edward Kawas <edward.kawas+owl2perl@gmail.com>
#-----------------------------------------------------------------
package semanticscience::org::resource::SIO_000500;

use OWL::Data::OWL::Class;
use semanticscience::org::resource::SIO_000532;
...
@ISA = qw(
 OWL::Data::OWL::Class
 semanticscience::org::resource::SIO_000532 
);
use strict;
...
use semanticscience::org::resource::SIO_000313;
...

{
    my %_allowed = (
        ...
        SIO_000313 => {
        	...
        	post => sub {
        		my ($self) = shift;
        		return unless $self->strict;
                my $type = @{$self->SIO_000313}[-1] if defined $self->SIO_000313;
                return unless defined $type and $type->type;
                my $range = new semanticscience::org::resource::SIO_000313->range();
                return unless $range;
                $range = $self->uri2package($range);
                eval {
                	$range = $range->new();
    ...
    sub _accessible {
        ...
        exists $_allowed{$attr} 
          ...
          or $self->semanticscience::org::resource::SIO_000532::_accessible($attr)
    }
    ...
    sub _attr_prop {
        ...
        return $self->semanticscience::org::resource::SIO_000532::_attr_prop( $attr_name, $prop_name ) 
            if $self->semanticscience::org::resource::SIO_000532::_accessible($attr_name);
...
sub init {
    ...
    $self->semanticscience::org::resource::SIO_000532::init();
    $self->type('http://semanticscience.org/resource/SIO_000500');
...
sub _get_statements {
    ...
    eval {
    	# add parent statements
        $self->semanticscience::org::resource::SIO_000532::_get_statements();
    };
    ...
    # for each attribute, if is array do:
    if (defined $self->SIO_000313){
	    foreach (@{$self->SIO_000313}) {
	        # add all statements from $_
	        my $enumerator = $_->_get_statements if defined $_->_get_statements;
	        next unless defined $enumerator;
            my $statement = $enumerator->getFirst;
	        ...
	        $self->model->addStmt(
	          new RDF::Core::Statement(
	              $subject,
	              $subject->new('http://semanticscience.org/resource/SIO_000313'), 
	              (defined $_->subject ? $_->subject : RDF::Core::Resource->new($_->value))
	          )
	        );
	        $add_type_statement = 1;
...
semanticscience::org::resource::SIO_000500 - an automatically generated owl class!

=cut

=head1 SYNOPSIS

  use semanticscience::org::resource::SIO_000500;
  my $class = semanticscience::org::resource::SIO_000500->new();
  ...
  # Make sure to use the appropriate OWL class! 
  # OWL::Data::OWL::Class used as an example
  $class->add_SIO_000313(new OWL::Data::OWL::Class('#someURI'));

  # get the SIO_000313 object properties
  my $SIO_000313_objects = $class->SIO_000313;
...
L<semanticscience::org::resource::SIO_000532>
